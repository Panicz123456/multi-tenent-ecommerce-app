...
export const checkoutRouter = createTRPCRouter({
  verify: protectedProcedures.mutation(async ({ ctx }) => {
    const user = await ctx.payload.findByID({
      ...
      depth: 0, //user.tenant[0] is going to be string
    });
    ...
    const tenantId = user.tenants?.[0]?.tenant as string; // this is id because of depth: 0
  ...
  purchase: protectedProcedures
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!tenant.stripeDetailsSubmitted) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "tenant allow to sell product",
        });
      }
