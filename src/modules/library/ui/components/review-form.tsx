...
export const ReviewForm = ({ productId, initialData }: Props) => {
  ...
  const createReview = useMutation(
    trpc.reviews.create.mutationOptions({
      onSuccess: () => {
        // automatically refetch review.getOne after create review
  ...
  const updateReview = useMutation(
    trpc.reviews.update.mutationOptions({
      onSuccess: () => {
        // automatically refetch review.getOne after update review
        queryClient.invalidateQueries(
  ...
  const onSubmit = (values: reviewSchemaType) => {
    if (initialData) {
      updateReview.mutate({
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormControl>
                <Textarea
                  placeholder="Want a leave a written review?"
                  disabled={isPreview}
                  {...field}
                />
              </FormControl>
...
export const ReviewFormSkeleton = () => {
  return (
    <div className="flex flex-col gap-y-4">
      ...
      <StarPicker disabled />
      <Textarea placeholder="Want a leave a written review?" disabled />
      <Button
        variant="elevated"
        disabled
        type="button"
